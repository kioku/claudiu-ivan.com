...
<Annotation>
  ...
  Code examples throughout this article reference helper functions from the
  companion modules: `option.ts`, `result.ts`, and `conversions.ts`.
</Annotation>
...
<Annotation>
  The `User | undefined` return type technically indicates potential absence,
  but relying on developers to consistently check creates fragility. With
  `strictNullChecks` enabled, TypeScript would catch the unsafe access above at
  compile time. However, many codebases operate without strict null checking,
  and even with it enabled, traditional patterns place the burden of correctness
  on vigilance rather than structure.
</Annotation>
...
<Annotation>
  This pattern—often called "Railway-Oriented Programming"—ensures that
  processing stops at the first error while automatically propagating context.
  Each `flatMap` only executes if the previous operation succeeded. If any
  validation fails, the error propagates to the final Result without executing
