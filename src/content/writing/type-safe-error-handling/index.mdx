...
description: "Exploring Option and Result types in TypeScript, where missing error handling becomes a compile error instead of a production incident."
date: "2026-01-20"
draft: false
---
...
<Annotation>
**[Try it in StackBlitz →](https://stackblitz.com/github/kioku/claudiu-ivan.com/tree/main/playground/result-option-types)** The companion code implements everything discussed in this article from scratch. You can step through the type definitions, run the examples, and modify them to see how the compiler responds to missing error handling.
</Annotation>
...
What makes Option useful is not the type itself but the operations: [`map`](https://github.com/kioku/claudiu-ivan.com/blob/4aed9fe/playground/result-option-types/option.ts#L54) transforms the inner value if present, [`flatMap`](https://github.com/kioku/claudiu-ivan.com/blob/4aed9fe/playground/result-option-types/option.ts#L66) chains operations that themselves return Options, and [`match`](https://github.com/kioku/claudiu-ivan.com/blob/4aed9fe/playground/result-option-types/option.ts#L100) forces exhaustive handling of both cases.
...
Result supports the same operations as Option, with the addition of [`mapErr`](https://github.com/kioku/claudiu-ivan.com/blob/4aed9fe/playground/result-option-types/result.ts#L69) for transforming errors while leaving success values alone.
...
The nested flatMap calls are not pretty. For production use, neverthrow provides method chaining ([`.andThen()`](https://github.com/kioku/claudiu-ivan.com/blob/4aed9fe/playground/result-option-types/result.ts#L84), [`.map()`](https://github.com/kioku/claudiu-ivan.com/blob/4aed9fe/playground/result-option-types/result.ts#L55), [`.mapErr()`](https://github.com/kioku/claudiu-ivan.com/blob/4aed9fe/playground/result-option-types/result.ts#L69)) that eliminates the nesting.
