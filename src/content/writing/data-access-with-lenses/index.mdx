...
The laws make good tests because they are easy to state and easy to violate. A broken lens is worse than no abstraction because it gives a bad update path a trustworthy name. Property-based testing fits naturally here: the laws are expressed over all valid inputs, not one hand-picked example. The companion code [exercises each law with `fast-check`](https://github.com/kioku/claudiu-ivan.com/blob/a357dc3/playground/data-access-with-lenses/lens-core.test.ts).
