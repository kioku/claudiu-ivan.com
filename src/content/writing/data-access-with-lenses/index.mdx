...
The runtime version of `view` returns a [`Result`](/writing/type-safe-error-handling) instead of pretending every path lookup succeeds:
...
interface Lens<S, A> {
  readonly view: (source: S) => Result<A, string>;
  readonly set: (source: S, newValue: A) => S;
}
```

The pure lens definition is shorter. The runtime version adds an error channel so callers cannot accidentally ignore a missing or mistyped path. A `string` error keeps the lens itself focused on a single concern; the formula engine in the next section carries a typed error union, and a production version would add the configured path, the offending segment, and the source type for richer logs.
