...
`S` is the source, the larger structure. `A` is the part being focused. The `Result` channel is small for property lenses, but it becomes important once paths come from runtime configuration and a lookup can fail.
...
The smallest useful building block is [`lensProp`](https://github.com/kioku/claudiu-ivan.com/blob/f952117dca95731aec13916cee23ed265f98a324/playground/data-access-with-lenses/lens-core.ts#L23), which focuses on a single property:
...
The implementation is small. [`composeLens`](https://github.com/kioku/claudiu-ivan.com/blob/f952117dca95731aec13916cee23ed265f98a324/playground/data-access-with-lenses/lens-core.ts#L41) extends its reach:
...
[`createLensFromConfig`](https://github.com/kioku/claudiu-ivan.com/blob/f952117dca95731aec13916cee23ed265f98a324/playground/data-access-with-lenses/lens-configurable.ts#L81) walks that path at runtime:
  ...
  return ok({
    view: (source) => {
      ...
      for (const key of config.getterPath) {
        const next = validatePathElement(current, key, config.getterPath);
    ...
    set: (source, newValue) => {
      // The full companion code uses Immer and validates intermediates
      // before writing so path failures return Err instead of throwing.
      return setPathWithImmer(source, config.getterPath, newValue);
    },
  });
}
```
...
[`evaluateFormula`](https://github.com/kioku/claudiu-ivan.com/blob/f952117dca95731aec13916cee23ed265f98a324/playground/data-access-with-lenses/formula-engine.ts#L88) loops over each token a formula requires, resolves it through a configured lens, and passes the results to the formula's `execute`:
...
function evaluateFormula(
  ...
  const inputs: Record<string, FormulaResultOutput> = {};
  ...
  for (const token of formula.getRequiredTokens()) {
    ...
    const lensResult = createLensFromConfig(config);
    if (isErr(lensResult)) {
      return err({ kind: "InvalidLensConfig", token, reason: lensResult.error });
    }

    const viewResult = lensResult.value.view(dataContext);
    if (isErr(viewResult)) {
      return err({ kind: "ResolutionFailed", token, reason: viewResult.error });
    }
...
The engine's own paths never throw. `FormulaError` distinguishes missing formula definitions, missing token configuration, invalid lens configuration, failed path resolution, and formula-author exceptions. Callers branch on the `Ok`/`Err` discriminant with `isOk` or `isErr`, and TypeScript flags any missed error case once they switch on `error.kind`.

Formulas hold the business logic, lens configuration locates the data, and validation catches bad mappings. Bad configuration can still corrupt results, which is why the runtime boundary needs aggressive validation, mapping tests, and a typed error channel rather than thrown exceptions.
