...
- repeat path knowledge across several callers,
...
Enterprise systems often have formula engines, configurable forms, import mappings, or client-specific data models. Hard-coding every path in application logic does not scale across products and clients. One common fix is to introduce **tokens**: stable names for domain concepts such as `IRS.Notional`, `Option.Strike`, or `Policy.Premium`. Formulas reference the token; configuration maps each token to a concrete path on the current data structure. A lens walks that path.
...
- Does `view` return a typed result, or does it fail another way?
