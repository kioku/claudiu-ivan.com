...
<Annotation>
**[Try it in StackBlitz →](https://stackblitz.com/github/kioku/claudiu-ivan.com/tree/main/playground)** The companion code implements typed lenses, runtime-configured lenses, lens-law property tests, and a small example of how a caller consumes tokens. You can step through the modules, run the tests, and modify the examples to see how the type system and the runtime checks respond.
</Annotation>
...
The boundary needs ownership. A token such as `IRS.Notional` may be referenced across formula definitions, reports, client mappings, operational tooling, and support documentation. A change to the underlying object shape can break all of them without changing a single formula. At that point, lens configuration should be managed as deployable system configuration, with ownership, versioning, validation, and runtime visibility.
