...
function evaluateFormula(
  ...
  for (const token of formula.getRequiredTokens()) {
    ...
    if (!config) {
      return { success: false, error: { kind: "MissingLensConfig", token } };
    }

    const result = createLensFromConfig(config).view(dataContext);
    if (!result.success) {
      return {
        success: false,
        error: { kind: "ResolutionFailed", token, reason: result.error },
      };
    }
  ...
  return { success: true, value: formula.execute(inputs) };
}
```

The function never throws. `FormulaError` distinguishes a missing configuration from a path that fails to resolve, and each variant carries the token. Callers handle both the success and failure arms, and TypeScript flags any missed case.
