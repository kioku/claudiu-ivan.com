...
In financial systems this shows up constantly. Interest rate products and valuation contexts are often nested several levels deep. The same pattern appears in claims systems, logistics tools, and product catalogs. Once the domain gets real, the objects stop being flat.
...
<Annotation>
[Try it in StackBlitz ->](https://stackblitz.com/github/kioku/claudiu-ivan.com/tree/main/playground). The companion code includes typed lenses, runtime-configured lenses, property tests, and a small token-consuming example. You can step through the modules, run the tests, and modify the examples to see how the type system and runtime checks respond.
</Annotation>
...
A production version needs clear behavior for missing paths, runtime type mismatches, and discriminated unions reached through configuration. It also needs to decide whether `set` can create missing structure or only update paths that already exist.

The companion implementation keeps the same `Lens<S, A>` shape introduced earlier. `view` returns `ViewResult<A>` and `set` returns `SetResult<S>`. Configuration errors surface as `Err` from `createLensFromConfig`. A production version would include enough context to identify which mapping failed and where traversal stopped.
...
A production setup should make ownership and compatibility explicit. Each token namespace needs an owner. Each mapping should target a known schema version. Representative fixtures should prove that the configured path still resolves. The team also has to decide whether failures block deployment or surface at request time.

The error context matters. `Could not resolve IRS.Notional` is better than `Cannot read property amount of undefined`, but it still may not be enough. A better failure should name the token, show the configured path, and include the expected type.
...
A practical cutoff is useful. Use object spread for shallow one-off updates. Use named lenses for repeated nested immutable access. Reach for prisms when the target may not exist because of the shape of the type. Use traversals when there may be many targets. If optics become central to the system, use a library.
