...
The formula engine resolves each token through a configured lens, then [`evaluateFormula`](https://github.com/kioku/claudiu-ivan.com/blob/20a35cf396d3c7216a577cae3846d03fc3b12e3b/playground/data-access-with-lenses/formula-engine.ts#L139) passes the resolved inputs to the formula's `execute`:
...
interface FormulaDefinition {
  ...
  readonly execute: (
    inputs: Record<string, FormulaResultOutput>
...
type FormulaError =
  ...
  | { readonly kind: "ExecutionFailed"; readonly formulaId: string; readonly reason: string };
...
The engine converts lookup, configuration, path-resolution, and formula execution failures into `Err`. Formula code returns `Result` directly; the example project does not need `try/catch`, and its formula definitions do not throw. Callers branch on the `Ok`/`Err` discriminant with `isOk` or `isErr`, and TypeScript flags any missed error case once they switch on `error.kind`.
