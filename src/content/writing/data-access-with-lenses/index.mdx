...
`ViewResult<A>` and `SetResult<S>` use the same `_tag: "Ok" | "Err"` discriminant as the shared `Result` type. A production version would extend each error with the configured path, the offending segment, and the source type. The pure lens definition is shorter, but the runtime version adds an error channel so callers cannot accidentally ignore a missing or mistyped path.

## Stable Names for Moving Paths

A formula, report, import mapping, or configurable form should not need to know where a value lives inside an `IRS` object.

```text
PresentValue = DiscountFactor * IRS.Notional
```

`IRS.Notional` is a token: a stable name for a business concept. The token can remain stable even if the object path changes.
...
If the schema later moves notional under `trade.economics.notional.amount`, the formula should not change. The token mapping changes:
