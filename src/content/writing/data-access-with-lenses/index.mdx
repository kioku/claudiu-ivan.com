...
With typed paths, `Result` mostly documents an unlikely failure. With configured paths, it carries real lookup failures. At that boundary, lookup can fail even when the TypeScript types compile.
...
A configured lens is the mechanism that resolves the token against a runtime object. It can fail because the token has no configuration, the configuration is malformed, or the configured path does not exist on the current object. A production version should also fail when the resolved value has the wrong runtime type.
...
The [formula example](https://github.com/kioku/claudiu-ivan.com/blob/20a35cf396d3c7216a577cae3846d03fc3b12e3b/playground/data-access-with-lenses/formula-engine.ts) is intentionally small. Formulas ask for tokens. Lenses resolve those tokens, and callers receive typed success or failure.
